...
Using Z Lab's DFlash speculative decoding models with SGLang’s new default Spec V2 engine, you can achieve state-of-the-art latencies for LLM inference serving.
...
Speculative decoding speedup depends on two factors: how many drafted tokens are accepted per cycle, and how much extra cost the draft model adds. DFlash improves both: diffusion drafting lowers draft cost, and KV injection raises acceptance.

For the end-to-end comparison, we use a 5-layer EAGLE-3 drafter as the baseline, matching the depth of the DFlash drafter. Concretely, DFlash achieves a similar acceptance length to a 5-layer EAGLE-3 drafter, but delivers much higher end-to-end speedup. Results are reported as `acc_len / speedup`. Both settings produce 7 draft tokens per cycle. The target model is Qwen3-4B, and both drafters are trained on the same 100K-sample dataset.
...
Autoregressive drafters like EAGLE-3 generate draft tokens one by one, so drafting cost grows roughly linearly with draft length. This usually forces shallow draft models, limiting draft quality.

DFlash avoids this bottleneck with a block diffusion drafter. It generates the whole block in one parallel forward pass, so latency grows much more slowly. The sweep below compares 4, 8, and 16 draft tokens. A 5-layer DFlash drafter generating 16 tokens has much lower drafting latency than a single-layer EAGLE-3 drafter.
...
This ablation isolates the effect of diffusion drafting: DFlash gives higher speedup even with lower acceptance length.
...
Fast drafting only helps if the drafted tokens are accepted. EAGLE-3 uses target-model features only at the draft input, so this signal can fade in deeper drafters.

DFlash injects target features into every draft layer’s KV cache, keeping the drafter aligned with the target model’s context throughout generation.

This ablation isolates KV injection: with autoregressive drafting, DFlash still runs faster by improving acceptance length.
...
We don’t want to store those latents: they would consume precious KV cache space, and requests with the same prefix should still share the radix cache. So we run the draft KV projection ahead of the rest of the draft forward pass – *immediate materialization*. That needs to be fast, so we added a layer-batched linear projection and a fused Triton kernel for the norm+RoPE post-processing.
...
These optimizations can be used by all draft models, but DFlash takes greater advantage of overlap scheduling. In particular, because DFlash uses immediate materialization from the target to construct the draft KV, it doesn’t need a separate draft-extend step to run the draft model on only accepted tokens and populate KV. This draft-extend step, used in EAGLE, requires that accepted tokens are known before host-side planning can proceed.
...
More broadly: you can run inference at state-of-the-art intelligence and speed thanks to the work of the open-weights model builders, systems researchers, and the open source community. Whether it’s research work on techniques like DFlash by [Z Lab](https://z-lab.ai/) or features and performance enhancements from open source contributors like [Modal](https://modal.com/), the world’s best work on LLM inference is landing in the SGLang open source engine for you to build on and with.
